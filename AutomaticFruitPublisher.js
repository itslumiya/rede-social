...
function recordLog(message, timestamp) {
  ...
  const logPath = path.join(logDir, `client-fruit-log-${dateToday}.txt`);
...
async function runClient() {
  ...
  while (true) {
    ...
    const message = {
      ...
      Message: `A fruta escolhida foi ${fruit}`,
      Username: "Fruta",
      Timestamp: timestamp
    };
    ...
    try {
      ...
      recordLog(`[Publicação] Fruta (${timestamp}) - ${message.Message}`, timestamp);
    } catch (err) {
